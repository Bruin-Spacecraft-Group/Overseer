...
class flight(object):
    ...
    Parameters
    ----------
    balloonGasType : string ('Helium'|'Hydrogen')
    balloonModel : string
        Accepted values can be found in `astra.available_balloons_parachutes.balloons`.
    nozzleLift : scalar
        nozzle lift in kg
    ...
    def __init__(self,
        ...
        if log_to_file:
            ...
            # while not interrupting with general use of astra.
    ...
    def launchSiteLat(self, _launchSiteLat):
        if _launchSiteLat == 0.0:
            logger.warning('Launch Site Latitude set to zero.')
        self._launchSiteLat = _launchSiteLat
    ...
    def launchSiteLon(self, _launchSiteLon):
        if _launchSiteLon == 0.0:
            logger.warning('Launch Site Longitude set to zero.')
        self._launchSiteLon = _launchSiteLon
